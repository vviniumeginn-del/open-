...
**何时生成**：
- 文章中提到多个材质/产品的对比
- 有明确的指标对比（耐磨、清洁难度、价格、寿命等）
- 数字或维度较多，纯文字不够直观

**示例触发条件**：
```markdown
# 文章中出现类似内容
"猫抓布耐磨等级高于科技布，但清洁难度更高..."
"真皮单价更高，但耐用性更强..."
"1.4m餐桌适合4人，1.6m适合6人..."
```

**生成内容**：横向对比柱状图、雷达图、前后对比图
...
```markdown
# 文章中出现类似内容
"沙发结构分为三层：框架层、支撑层、面料层..."
"生产流程：选材 → 开料 → 组装 → 打磨 → 涂装..."
"餐桌结构包括：桌面、围板、桌腿与连接件..."
```

**生成内容**：结构示意图、流程图、层次结构图
...
**提示词模板**：
```
A clean comparison chart for [主题].
Design: horizontal bar chart with 3-4 bars, each representing a different [材质/产品].
...
Elements:
- Y-axis: material/product names in simplified Chinese ([选项1], [选项2], [选项3])
- X-axis: metric in Chinese ([指标名称，如"耐磨等级"、"价格区间(元)"、"清洁难度"])
...
**提示词模板**：
```
A modern structure diagram for [产品名称].
Design: layered structure with 3-4 tiers, connected by arrows showing support layers.
...
Visual elements:
- Arrows showing support flow (with Chinese labels like "承重", "支撑")
...
**提示词模板**：
```
A clear comparison image showing [风格A] vs [风格B].
Layout: split screen design, left side for [风格A], right side for [风格B].
Design:
...
Text labels:
- Left: "[风格A名称]" in Chinese
- Right: "[风格B名称]" in Chinese
...
```
[风格A] = "北欧风"
[风格A的视觉表现] = "light wood, white walls, minimal decor"
[风格B] = "原木风"
[风格B的视觉表现] = "warm wood textures, earthy tones, cozy lighting"
```
...
- [维度5，如"价格"]
Products (lines):
- [选项1]: blue line (#3b82f6)
- [选项2]: beige line (#c7b299)
- [选项3]: green line (#10b981)
Visual elements:
...
在耐磨和打理难度上，三种面料差异明显。猫抓布更耐磨，
科技布更好打理，真皮质感更好但成本更高。

![材质对比图](material-comparison.png)

*图：常见沙发面料耐磨/清洁对比*

根据家庭情况和预算选择，更容易买到适合的款式。
```
